...
def render_upload_page():
    ...
    # This section is gated behind a password to prevent abuse of the live AWS backend, which incurs costs. 
    # The password is stored securely in Streamlit secrets and can be shared with recruiters upon request.
    if not st.session_state.password_correct:
        st.info("🔒 **Private Demo**: Live analysis is protected to manage AWS costs and prevent abuse. Message me on LinkedIn for the password.")
        pwd_input = st.text_input("Enter password to unlock upload:", type="password")
        ...
        st.stop() # block the rest of the page if not authenticated 
...
if st.session_state.password_correct:
    st.sidebar.success("Status: Authenticated (Live)")
    if st.sidebar.button("Logout (Lock Live Mode)"):
        st.session_state.password_correct = False
        st.rerun()
else:
    st.sidebar.warning("Status: Demo Mode (Locked)")
